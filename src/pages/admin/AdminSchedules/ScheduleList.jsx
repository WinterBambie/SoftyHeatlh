...
import {
  scheduleDayLabel, scheduleTimeRange,
  scheduleMaxPatients, scheduleReserved,
...
function ScheduleList({ schedules, onRefresh, onEdit, onDelete }) {
  if (!schedules?.length)
    return (
      ...
      <div style={{
        ...
        {typeof onRefresh === "function" && (
          <button onClick={onRefresh} style={{
            ...
            <Refresh fontSize="small" /> Actualizar
          </button>
        )}
      </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "14px", minWidth: "720px" }}>
          <thead>
            <tr style={{ backgroundColor: COLORS.btnBg }}>
              {["#","Doctor","Sesión","Día","Franja horaria","Cita (min)","Cupos","Estado","Acciones"].map(h => (
                <th key={h} style={{
            ...
            {schedules.map((sc, i) => {
              ...
              return (
                <tr key={sc.scheduleid} style={{
                  ...
                  backgroundColor: i % 2 === 0 ? "#fff" : COLORS.gray,
                }}>
                  <td style={{ padding: "0.75rem 1rem", color: COLORS.muted }}>{sc.scheduleid}</td>
                  ...
                  <td style={{ padding: "0.75rem 1rem" }}>
                    <span style={{
                      ...
                      {Number.isFinite(maxNum) ? (full ? "Completo" : "Disponible") : "—"}
                    </span>
                  </td>
                  <td style={{ padding: "0.75rem 1rem" }}>
                    <div style={{ display: "flex", gap: "6px" }}>
                      <button onClick={() => onEdit(sc)} title="Editar"
                        style={iconBtn("#1565C0", "#E3F2FD", "#BBDEFB")}>
                        <Edit style={{ fontSize: "16px" }} />
                      </button>
                      <button onClick={() => onDelete(sc.scheduleid)} title="Eliminar"
